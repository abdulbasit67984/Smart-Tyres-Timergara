...
const seedDefaultAccounts = async (BusinessId) => {
    const accountNames = ["Asset", "Liability", "Owner Equity", "Expense", "Revenue"];
    const subCategories = [
        { name: "Current Asset", parent: "Asset" },
        { name: "Current Liability", parent: "Liability" },
        { name: "Expense", parent: "Expense" },
        { name: "Revenue", parent: "Revenue" }
    ];
    const individualAccounts = [
        { name: "Inventory", parentSubCategory: "Current Asset" },
        { name: "Cash", parentSubCategory: "Current Asset" },
        { name: "Account Receivables", parentSubCategory: "Current Asset" },
        { name: "Account Payables", parentSubCategory: "Current Liability" },
        { name: "Sales Revenue", parentSubCategory: "Revenue" },
        { name: "General Expense", parentSubCategory: "Expense" }
    ];
    ...
    const existingAccountMap = new Map(existingAccounts.map((acc) => [acc.accountName, acc]));

    for (const accountName of accountNames) {
        if (!existingAccountMap.has(accountName)) {
            const account = await Account.create({ BusinessId, accountName });
            existingAccountMap.set(accountName, account);
        }
    }

    const existingSubCategories = await AccountSubCategory.find({
        BusinessId,
        accountSubCategoryName: { $in: subCategories.map((sub) => sub.name) }
    });

    const existingSubCategoryMap = new Map(
        existingSubCategories.map((sub) => [sub.accountSubCategoryName, sub])
    );

    for (const subCategory of subCategories) {
        if (!existingSubCategoryMap.has(subCategory.name)) {
            const parentAccount = existingAccountMap.get(subCategory.parent);
            if (!parentAccount) {
                throw new ApiError(500, `Parent account not found for ${subCategory.name}`);
            }
            const createdSubCategory = await AccountSubCategory.create({
                BusinessId,
                accountSubCategoryName: subCategory.name,
                parentAccount: parentAccount._id
            });
            existingSubCategoryMap.set(subCategory.name, createdSubCategory);
        }
    }

    const existingIndividualAccounts = await IndividualAccount.find({
        BusinessId,
        individualAccountName: { $in: individualAccounts.map((acc) => acc.name) }
    });

    const existingIndividualAccountSet = new Set(
        existingIndividualAccounts.map((acc) => acc.individualAccountName)
    );

    for (const individualAccount of individualAccounts) {
        if (existingIndividualAccountSet.has(individualAccount.name)) {
            continue;
        }

        const parentSubCategory = existingSubCategoryMap.get(individualAccount.parentSubCategory);
        if (!parentSubCategory) {
            throw new ApiError(500, `Parent subcategory not found for ${individualAccount.name}`);
        }

        await IndividualAccount.create({
            BusinessId,
            individualAccountName: individualAccount.name,
            parentAccount: parentSubCategory._id
        });
    }
}
...
const registerBusiness = asyncHandler(async (req, res) => {
    ...
    await seedDefaultAccounts(business._id);
